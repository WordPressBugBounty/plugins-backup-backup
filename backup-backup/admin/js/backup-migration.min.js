...
jQuery(window).on('load', function() {
  if (pagenow !== 'toplevel_page_backup-migration' && pagenow !== 'toplevel_page_backup-migration-network') return;
  setTimeout(function() {
    jQuery('#bmi').css({
      opacity: 0
    });
    ...
    bmi: {
      ...
      tooltips: {
        ...
        init: function() {
          ...
          const hideTooltips = function() {
            if (!isHidingTooltips && document.querySelector('.tooltipster-base')) {
              isHidingTooltips = true;
              $.bmi.tooltips.hideAll(true);
              setTimeout(() => { isHidingTooltips = false; }, 500);
            }
          };
          window.addEventListener('scroll', hideTooltips, true);
          window.addEventListener('resize', hideTooltips, true);
      ...
      fillWithNewBackups: function(res, ongoing, cb = function() {}, search = '') {
        ...
        for (; i < backupsSorted.length; ++i) {
          ...
          $template.find('.bc-download-btn').attr('href', $('#BMI_BLOG_URL').text().trim() + '/?backup-migration=BMI_BACKUP&bmi-id=' + name + '&sk=' + $('#BMI_DOWNLOAD_TOKEN').text().trim());
          ...
          if ($('#BMI_BACKUP_PRO').val() == 1) {
            
            $template.attr('data-backup-name', name);
            console.log("backups[index]", backups[index])
            let isEncrypted = (backups[index].is_encrypted === true || backups[index].is_encrypted === "true" || backups[index].is_encrypted == 1);
            if (isEncrypted) {
              $template.find('.bc-encrypt-btn').hide();
              $template.find('.bc-decrypt-btn').show();
            } else {
              $template.find('.bc-encrypt-btn').show();
              $template.find('.bc-decrypt-btn').hide();
            }
          }
      ...
      showMoreBackups: function(res) {
        ...
        backups = {};
        let trs = $('#bmi_restore_tbody').find('tr:hidden').not('.bmi-empty-text').not('.bmi-empty-text-search').not('.search-hidden');
      ...
      searchInBackups: function(keyword, cb = function() {}) {
        let keywordLower = keyword.toLowerCase().trim();
        let $rows = $('#bmi_restore_tbody').find('tr').not('.bmi-empty-text').not('.bmi-empty-text-search');
        let matchCount = 0;
        
        $rows.each(function() {
          let name = $(this).find('.br_name').text().toLowerCase();
          $(this).hide(); // Reset to hidden for pagination
          if (keywordLower === '' || name.indexOf(keywordLower) !== -1) {
        ...
        if (matchCount === 0) {
          if ($('#bmi_restore_tbody').find('tr.bmi-empty-text-search').length === 0) {
            let empty = $('#bmi_restore_tbody').data('empty') || 'No backups found matching your search.';
            $('#bmi_restore_tbody').append('<tr class="bmi-empty-text-search"><td class="center text-muted" colspan="100%">' + empty + '</td></tr>');
          } else {
            $('#bmi_restore_tbody').find('tr.bmi-empty-text-search').show();
          }
          $('#load-more-backups-wrp').hide();
        } else {
          $('#bmi_restore_tbody').find('tr.bmi-empty-text-search').hide();
          $.bmi.showMoreBackups();
        }
  ...
  $.bmi.crons = function() {
    ...
    function settings_changed() {
      ...
      let settings = {
        ...
        remote_ping: ($('#remote-ping').prop('checked') ? 'true' : 'false')
      }
      ...
      getCronResult(settings, function(res) {
        ...
        if (res.status === 'success' && res !== false) {
          ...
          if (!firsttime){
            if (res.local_site) {
              $.bmi.alert('warning', $('#bmi-cron-local-site').text(), 30000);
            } else if (!res.ping_working) {
              $.bmi.alert('warning', $('#bmi-cron-ping-server').text(), 30000);
            } else {
    ...
    $('#bmi').on('change', '#remote-ping', function(e) {
...
jQuery(document).ready(function ($) {
    ...
    $.bmi
      ...
      .then(function (res) {
        ...
        if (res.status == "success") cb(false);
          ...
          if (mute == true && res.status == "msg") cb(true);
          else {
            $.bmi.modal("freeze-loading-modal").close();
            $.bmi._msg(res);
          }
  ...
  $("#bmi_restore_tbody").on("click", ".bc-remove-btn", function (e) {
    ...
    latest_delete[name] = {
      hash: $el.closest("tr").getAttribute("md5"),
      isCloud: isCloud
    };
  ...
  $("#sure_delete").on("click", function (e) {
    ...
    // Reset/Setup delete progress modal
    $("#delete-progress-modal .progress-active-bar").css("width", "0%");
    $("#delete-progress-modal .progress-percentage").css("left", "0%").text("0%");
    $("#delete_current_step").text("Preparing deletion...");
    $("#cancel_delete").removeClass("disabled").removeAttr("disabled").css("pointer-events", "").css("opacity", "");
    
    $.bmi.modal("delete-progress-modal").open();

    let totalBackups = Object.keys(latest_delete).length;
    ...
    $("#cancel_delete").off("click").on("click", function(event) {
      ...
      isCancelled = true;
      $.bmi.modal("delete-progress-modal").close();
      $.bmi.reloadBackups();
      $.bmi.alert("warning", "Backup deletion was cancelled.", 3000);
    });

    let processDeletions = function(backupsList) {
      if (isCancelled) return;

      // Show which file is being deleted
      let currentFile = Object.keys(backupsList)[0];
      if (currentFile) {
        $("#delete_current_step").text("Deleting " + currentFile + "...");
      }

      // If it's the last batch, disable Cancel
      if (Object.keys(backupsList).length <= 5) {
        $("#cancel_delete").addClass("disabled").attr("disabled", true).css("pointer-events", "none").css("opacity", "0.5");
      }
      ...
      $.bmi
        ...
        .then(function (res) {
          ...
          if (res.status == "continue" && res.remaining) {
            let remainingCount = Object.keys(res.remaining).length;
            deletedBackups = totalBackups - remainingCount;
            let percent = Math.round((deletedBackups / totalBackups) * 100);
            
            $("#delete-progress-modal .progress-active-bar").css("width", percent + "%");
            $("#delete-progress-modal .progress-percentage").css("left", percent + "%").text(percent + "%");
            
            processDeletions(res.remaining);
          } else {
            // Completed
            $("#delete-progress-modal .progress-active-bar").css("width", "100%");
            $("#delete-progress-modal .progress-percentage").css("left", "100%").text("100%");
            $("#delete_current_step").text("Finished!");
    ...
    processDeletions(latest_delete);
  });
  ...
  function saveBtnEventHandler(e = false, type = false, cb = () => { }) {
    ...
    if (save == "save-storage") {
      ...
      data["direct_cloud_streaming"] = isChecked("direct-cloud-streaming"); // STREAM:DIRECT_CLOUD_STREAMING:ENABLED
      const selected = document.querySelector(
        'input[name="direct_cloud_provider"]:checked'
      );

      const value = selected ? selected.value : null;
      data["direct_cloud_provider"] = value; // STREAM:DIRECT_CLOUD_STREAMING:PROVIDER
      data["storage_strategy"] = document.querySelector('input[name="backup_storage_strategy"]:checked')?.value; // STORAGE:STRATEGY
      data["encryption"] = $('input[name="encryption"]:checked').val();
      data["encryption_password"] = $('input[name="encryption_password"]').val();
  ...
  function getEndCode() {
    let endCode = document.querySelector('.hide_so_much');
    if (endCode && endCode.innerText) {
      let code_line = endCode.innerText.trim();
      const result = code_line.match(/#(.+)$/)?.[1];
      return result || false;
    } else return false;
  }
  ...
  function insertPre(log, el) {
    ...
    latestLines = lines;
    if (lines.length >= 1) {
      applyActionBasedOnLogLines(lines);
    }
    ...
    setTimeout(function () {
      ...
      $.bmi
        ...
        .then(function (res_latest) {
          ...
          $("#bmi-streamed-backup-name")[0].innerText = res_latest;
          ...
          if ($("#backup-progress-modal").hasClass("open")) {
            ...
            if ($('#storage-encryption-true').is(':checked')) {
              $('.bmi-encryption-warning-wrapper').show();
            } else $('.bmi-encryption-warning-wrapper').hide();
  ...
  async function middlewareForResponses(res) {
    // Variables
  ...
  function runCheckOnGoing() {
    checkIfBackupOnGoing(function () {
      setTimeout(function () {
        if (!$("#freeze-loading-modal").hasClass("open") && !(tryingToAbortBackup)){
          runCheckOnGoing();
        }
      }, ongoing_interval);
    });
    ...
    $.bmi
      ...
      .then(function (res) {
        if (res.status === "cli") {
          ...
          console.log("Password required to proceed with restore. Prompting user for password.");
          promptAndValidatePassword(name)
            .then((validSecret) => {
              console.log("Password validated successfully. Resuming restore process.");
              res.options.step = parseInt(res.options.step) + 1;
              res.options.password = validSecret;
              
              runRestoreProcess(
                name,
                remote,
                "secret", // Bypass reset of logs and progress since we're just resuming after password entry
                res.tmp,
                "true",
                res.options
              );
            ...
            .catch((err) => {
              ...
              // Handled if the user clicks cancel
              console.warn(err.message);
              $.bmi.modal("restore-progress-modal").close();
              $("#bmi-force-restore-to-stop").click(); // Force stop the restore process on the backend as well
              setTimeout(function () { // Reset progress bar after a short delay to ensure the modal has closed first
                $("#restore-progress-modal .progress-active-bar")[0].style.width =
                  0 + "%";
                $("#restore-progress-modal .progress-percentage")[0].style.left = 0 + "%";
                $("#restore-progress-modal .progress-percentage")[0].innerText = 0 + "%";
  ...
  function promptAndValidatePassword(backupName) {
    console.log("Prompting user for backup password for backup:", backupName);
    return new Promise((resolve, reject) => {
      $("#bmi-password-modal-title").text($("#bmi-pwd-modal-restore-title").text().trim() || "Enter Backup Password");
      $("#bmi-password-modal-desc").text($("#bmi-pwd-modal-restore-desc").text().trim() || "Please provide the password for this backup to proceed with the restoration.");
      $("#backup-password-verify-confirm").text($("#bmi-pwd-modal-restore-btn").text().trim() || "Verify & Restore");
      $("#bmi-password-modal-warning-wrapper").hide();
      $("#bmi-password-modal-icon-box").html('<svg class="bmi-restore-password-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"></path></svg>');

      $.bmi.modal("password-request-modal").open();

      $("#bmi-backup-password-input").val("");

      $("#backup-password-verify-confirm").off("click").on("click", function (e) {
        e.preventDefault();
        
        const enteredPassword = $("#bmi-backup-password-input").val();
        
        $.bmi.modal("freeze-loading-modal").open();

        $.bmi
          .ajax("validate-backup-password", {
          ...
          .then(function (valRes) {
            ...
            if (valRes.status === "success") {
              $.bmi.alert("success", valRes.message || "Password correct! Resuming restore...", 3000);
              $.bmi.modal("password-request-modal").close();
              
              resolve(enteredPassword); 
            } else {
              $.bmi.alert(valRes.level || "error", valRes.why || "Incorrect password, please try again.", 3000);
            }
          })
          .catch(function (error) {
            console.error("Password validation error:", error);
            $.bmi.modal("freeze-loading-modal").close();
            $.bmi.alert("error", "An error occurred while validating the password. Please try again.", 3000);
          });
      });

      $("#password-request-modal .bmi-modal-close, #password-request-modal .bmi-modal-closer").off("click").on("click", function() {
        $.bmi.modal("password-request-modal").close();
  ...
  $(".deleteAllSelected").on("click", function () {
    ...
    for (let i = 0; i < $selected.length; ++i) {
      ...
      let isCloud = Array.from(tr.querySelectorAll('[class*="strg-"]')).filter(el => !el.classList.contains('strg-local')).some(el => el.classList.contains('img-green'));
      if (isCloud) anyInCloud = true;
      names.push(name);
    ...
    if (anyInCloud) $(".bmi-cloud-removal").show();
    else $(".bmi-cloud-removal").hide();
    $("#remove-cloud-backup-as-well")[0].checked = false;

    if (anyInCloud && notOnLocal.length === $selected.length) {
      $("#remove-cloud-backup-as-well")[0].checked = true;
      $(".bmi-cloud-removal").hide();
      $("#delete-confirm-modal").find(".text1").hide();
      $("#delete-confirm-modal").find(".text2").hide();
  ...
  function applyActionBasedOnLogLines(lines) {
    if ($("#backup-stop").hasClass("disabled")) {
      let backupInitializedLine = lines.find(line => line.includes("backup_initialized"));
      if (backupInitializedLine) {
        $("#backup-stop").removeClass("disabled");
      }
    }
  }
...
jQuery(document).ready(function($) {
  ...
  // Init tooltips
  if(pagenow === 'toplevel_page_backup-migration' || pagenow === 'toplevel_page_backup-migration-network')
    $.bmi.tooltips.init();
